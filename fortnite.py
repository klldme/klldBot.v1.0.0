...
async def event_ready():
    os.system('cls||clear')
    print(intro)
    print(Fore.GREEN + ' [Success] ' + Fore.RESET + 'Dein Bot ist Jetzt Online! ' + Fore.LIGHTBLUE_EX + f'{client.user.display_name}')
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        ...
        if invite.sender.id in info['FullAccess']:
            await invite.accept()
            print(Fore.BLUE + ' [Console] ' + Fore.RESET + 'Partyeinladung akzeptiert von ' + Fore.LIGHTGREEN_EX + f'{invite.sender.display_name}')
        else:
...
#More Commands By LeakerByDragon ;)
...
async def Tbd(ctx, *, content = None):
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(asset='CID_NPC_Athena_Commando_M_CatBurglar_Ghost')
    await asyncio.sleep(4.0)
...
async def klld(ctx, *, content = None):
    if content is None:
        ...
        await client.party.me.set_outfit(asset='CID_A_201_Athena_Commando_M_TeriyakiFishToon')
        await asyncio.sleep(1.0)
        await client.party.me.set_backpack(asset='BID_991_LyricalFemale')
        await asyncio.sleep(1.0)
        await client.party.me.set_emote(asset='EID_Robot')
        await asyncio.sleep(5.0)
        await client.party.me.set_emote(asset='EID_BuffCatComic_EV4HK')
...
async def SpecialCommand(ctx, *, content = None):
    if content is None:
        await ctx.send(f'{prefix}klld , {prefix}Randomize, {prefix}Spam, {prefix}Hack, {prefix}og, {prefix}Exclusive, {prefix}RE, {prefix}Toxic, {prefix}Rdw, {prefix}Johnnysins, {prefix}Info, {prefix}battlepassemotes ')
...
async def Info(ctx, *, content = None):
    if content is None:
        ...
        await ctx.send(f'Bot Made By klld')
        await ctx.send(f'')
        await ctx.send(f'klld.glitch.me')
...
async def Toxic(ctx, *, content = None):
    if content is None:
        await client.party.me.set_outfit(asset='ID: CID_A_201_Athena_Commando_M_TeriyakiFishToon')
        await asyncio.sleep(2.0)
        await client.party.me.set_emote(asset='EID_Robot')
        await asyncio.sleep(2.0)
